...
If(${env:scripty.auCollege} -eq $true) {
    ...
    if (-Not ( Test-Path "${env:scripty.cachePath}\Sdk.7z" )) {
        Write-Host '    Pas de SDK trouvÃ© en cache. Il va falloir construire'
        ...
        if (-Not ( Test-Path "$HOME\AppData\Local\Android\Sdk" )) {
            ...
            Start-Script "${env:scripty.scriptPath}\sdk-installe.ps1"
        }
        else {
            Write-Host '   �  Android SDK déjà copié et installé.' -ForegroundColor Green
        }
    }
}
Else{
